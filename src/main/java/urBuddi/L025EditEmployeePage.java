...
public class L025EditEmployeePage {
	...
	/*	Thread.sleep(2000);
		String bankNameRandomText = generateRandomString(5);
		enterBankName(bankNameRandomText);
		
		Thread.sleep(2000);
		int bankAccountRadomNumber = randomNumber.nextInt(1000000000);
		String bankNumberInput = "1"+bankAccountRadomNumber;
		enterBankAccountNumber(bankNumberInput);

		Thread.sleep(2000);
		bankIFSCRadomNumber = randomNumber.nextInt(1000000000);
		bankIFSCNumberInput = "OW"+bankIFSCRadomNumber;
		enterBankIFSCNumber(bankIFSCNumberInput);
		
		Thread.sleep(2000);
		panRadomNumber = randomNumber.nextInt(1000000000);
		panNumberInput = "OW"+panRadomNumber;
		enterPANNumber(panNumberInput);
	*/	
		Thread.sleep(2000);
		clickOnSubmitButton();
	...
	public void verifyEditEmployeeDetailsWithEditeedFirstAndLastName() throws InterruptedException {
		Thread.sleep(5000);		
		By employeeSearchFirstAndLastNameValue = By.xpath("//*[text()='"+editEmployeeFName+" "+editEmployeeLName+"']");
		Boolean afterEditEmployeeWithFirstAndLastNameEditedDetailsIsDisplayed = cm.isElementDisplayed(employeeSearchFirstAndLastNameValue);
		System.out.println("After Edit Employee First And Last Name With Edited Details Is Displayed?==" + afterEditEmployeeWithFirstAndLastNameEditedDetailsIsDisplayed);
		Assert.assertTrue(afterEditEmployeeWithFirstAndLastNameEditedDetailsIsDisplayed, "After Edit Employee First And Last Name With Edited Details Is Not Showing");		
	}
